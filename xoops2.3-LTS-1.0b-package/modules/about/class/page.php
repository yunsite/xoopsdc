...
class AboutPage extends XoopsObject
{
    function __construct() {
        ...
        $this->initVar('page_pid', XOBJ_DTYPE_INT, 0, false);
...
class AboutPageHandler extends XoopsPersistableObjectHandler
{
    ...
    {
        $pid = intval($pid);
        if (!is_array($tags) || count($tags) == 0) $tags = array("page_id","page_pid", "page_title", "page_title", "page_menu_title", "page_status", 'page_order');
    ...
    function &MenuTree($pages = array(), $key = 0, $level = 1){
        ...
        foreach($pages as $k=>$v){
            if($v['page_pid'] == $key){
                ...
                if(!empty($child)) $menu[$k]['child'] = $child;
            }
        }
