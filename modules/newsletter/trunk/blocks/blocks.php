...
function newsletter_block_service_show(){
    
    $block['service_id'] = isset($_REQUEST['service_id']) ? intval($_REQUEST['service_id']) : '';
    
    $service_handler = xoops_getmodulehandler('service','newsletter');
...
function newsletter_block_case_show($options){    
    
    global $xoopsDB;
    
    $service_handler = xoops_getmodulehandler('service','newsletter');
    $case_handler = xoops_getmodulehandler('case','newsletter');
    $cs_handler = xoops_getmodulehandler('cs','newsletter');
    $images_handler = xoops_getmodulehandler('images','newsletter');
    ...
    if($options[12] == 0) {  
        ...
        $query  = "SELECT * FROM " . $xoopsDB->prefix("newsletter_case");
    ...
    if(!empty($cases)) {
        ...
        foreach ($cases as $k=>$v) {
            ...
            if(!empty($options[8])) {
                $text = strip_tags($myts->undoHtmlSpecialChars(strip_tags($v['case_summary'])));
                $cases[$k]['case_summary'] = xoops_substr($text, '', $options[8]);
            }
            
            if(!empty($options[9])) {
                $text = strip_tags($myts->undoHtmlSpecialChars(strip_tags($v['case_description'])));
                $cases[$k]['case_description'] = xoops_substr($text, '', $options[9]);
            }
        }
...
function newsletter_block_case_edit($options){
	
    include_once XOOPS_ROOT_PATH."/modules/newsletter/class/blockform.php";
