 ...
 */
function newsletter_getTemplate($page = "index", $style = null)
{
    global $xoops;
 ...
 * 
 * The hardcoded path is not desirable for theme switch, however, we have to kenewsletter it before getting a good solution for cache
 *
 * @var string     $style
 *
 * @return string    file URL, false if not found
 */
function newsletter_getCss($style = "default")
{
    global $xoops;
 ...
 * @return string
 */
function newsletter_getModuleHeader($style = "default")
{
    $xoops_module_header = "<link rel=\"stylesheet\" type=\"text/css\" href=\"" . newsletter_getCss($style) . "\" />";
 ...
 * @return array
 */
function &newsletter_getTplPageList($page = "", $refresh = true)
{
    $list = null;
    ...
    */
    ...
    if ( !is_array($list) || $refresh ) {
        $list = newsletter_template_lookup(!empty($page));
    }
...
function &newsletter_template_lookup($index_by_page = false) 
{
