...
if($user['level'] == 'support') {
    if($user['level'] == 'support' && $process_obj->getVar('status') != 'create' && $process_obj->getVar('status') != 'forword') {
        if($user['uid'] != $process_obj->getVar('support_id')) redirect_header('index.php', 3, _MA_SUPPORT_NOPEREXPIRED);
    }
}
 
//if($user['level'] == 'support' && $user['uid'] != $process_obj->getVar('support_id') && $process_obj->getVar('status') != 'create' && $op != 'forword') redirect_header('index.php', 3, _MA_SUPPORT_NOPEREXPIRED);

// update process status and update log
if( $op == 'read' || $op == 'reply' || $op == 'reject' || $op == 'forword' || $op == 'finish' || $op == 'lgnore' || $op == 'close' ){
    // read                 
    if($op == 'read') {
        if($process_obj->getVar('status') != 'create' && $process_obj->getVar('status') != 'forword' && $process_obj->getVar('support_id') > 0) redirect_header('info.php?pro_id='.$pro_id, 3, _MA_SUPPORT_QUESTIONREAD);
    ...
    // status
    if($op != 'reply' && $op != 'reject' && $op != 'reject') $process_obj->setVar('status', $op);    
     
    $process_obj->setVar('update_time', time());
    ...
    if ($pro_id = $process_handler->insert($process_obj)) {
        ...
        $transform_obj->setVar('create_time', time());
