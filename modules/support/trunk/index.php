...
switch ($op) {
    ...
    case 'display':
        ...
        if($user['level'] == 'support') {
            ...
            $cat_criteria->add(new Criteria("cat_id","(".implode(", ",$catIds). ")","in"), 'AND');
            $cat_criteria->add(new Criteria('cat_status', 1));
            $catIds = $category_handler->getIds($cat_criteria);
        } else {
            $cat_criteria = new Criteria('cat_status', 1);
            $catIds = $category_handler->getIds($cat_criteria);  
        }
        ...
        if(!empty($process)) {
            ...
            foreach ($process as $k=>$v) {
                $process[$k]['status'] = '';
                $process[$k]['create_time'] = formatTimestamp($v['create_time'], $date);
                $process[$k]['update_time'] = formatTimestamp($v['create_time'], $date);
                ...
                $cat_ids[] = $v['cat_id'];
                $pro_ids[] = $v['pro_id'];
                if($v['status'] == 'forword' && $user['level'] == 'support' && $user['uid'] != $v['support_id'] && $v['support_id'] != 0) unset($process[$k]);
                if($v['status'] != 'create' && $v['status'] != 'forword' && $user['level'] == 'support' && $user['uid'] != $v['support_id']) unset($process[$k]);
            }
