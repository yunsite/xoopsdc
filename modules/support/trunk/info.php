...
if($process['status'] != 'create' && $process['status'] != 'forword' && $user['level'] == 'support') {
    if($process['support_id'] != $user['uid']) redirect_header('index.php', 3, _MA_SUPPORT_NOQUESTION); 
} 
...
switch ($op) {
    ...
    case 'info':
        ...
        // get times
        $process['create_time'] = formatTimestamp($process['create_time'], $date);
        ...
        if($user['level'] == 'customer') {                         
            ...
            if ($process['status'] == 'finish' || $process['status'] == 'lgnore' || $process['status'] == 'close') array_push($process['usesub'], 'reject', 'finish', 'lgnore');
            if ($process['status'] == 'create' || $process['status'] == 'forword') array_push($process['usesub'], 'reply','forword','finish', 'lgnore', 'close');
        } elseif ($user['level'] == 'support') {
            if ($process['status'] != 'create' && $process['status'] != 'forword' ) array_push($process['usesub'], 'read');
            if ($process['status'] == 'create' || $process['status'] == 'forword') array_push($process['usesub'], 'reply','forword','finish', 'lgnore', 'close');
            ...
            if ($process['status'] != 'create' || $process['status'] != 'forword' || $process['status'] == 'read') array_push($process['usesub'], 'read');
            if ($process['status'] == 'finish' || $process['status'] == 'lgnore' || $process['status'] == 'close') array_push($process['usesub'], 'reply', 'forword', 'finish', 'lgnore', 'close');   
        }
        array_unique($process['usesub']);
//        xoops_result($process);
        ...
        if($user['level'] == 'customer') {
            ...
            if ($process['status'] == 'read' || $process['status'] == 'reply') array_push($process['usesub'], 'read');
            if ($process['status'] == 'finish' || $process['status'] == 'lgnore' || $process['status'] == 'close') array_push($process['usesub'], 'read', 'reply', 'forword', 'finish', 'lgnore', 'close');
        } else {
            $process['usesub'] = array();
        ...
        */
        ...
        if(count($tranforms) > 1) {
            ...
            foreach($tranforms as $k=>$v) {
                ...
                $tranforms[$k]['create_time'] = formatTimestamp($v['create_time'], $date);
