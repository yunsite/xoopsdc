...
function list_blocks()
{
    ...
    foreach (array_keys($block_arr) as $i) {
        ...
        $sel0 = $sel1 = $ssel0 = $ssel1 = $ssel2 = $ssel3 = $ssel4 = $ssel5 = $ssel6 = $ssel7 = $sself = "";
        ...
        } elseif ( $block_arr[$i]->getVar("side") == XOOPS_BLOCK_CALLBACK ) {   // ezsky hack
            $sself = " checked='checked'";                                         // ezsky hack
        } 
                                                                            // ezsky hack
        ...
        <td class='$class' align='center' nowrap='nowrap'>
            ...
            <div style='text-align:left;border:1px dotted #ccc; margin:5px 0 0 0; padding:3px;'><input type='radio' name='side[$i]' value='".XOOPS_BLOCK_CALLBACK."'$sself />"._AM_FREEBLOCK."</div>
        </td>
    ...
    $block = array('form_title' => _AM_ADDBLOCK, 'side' => 0, 'weight' => 0, 'visible' => 1, 'title' => '','name' => '', 'content' => '', 'modules' => array(-1), 'is_custom' => true, 'ctype' => 'H', 'cachetime' => 0, 'op' => 'save', 'edit_form' => false);
    include XOOPS_ROOT_PATH.'/modules/system/admin/blocksadmin/blockform.php';
    $form->display();
}

function save_block($bside, $bweight, $bvisible, $btitle, $bname, $bcontent, $bctype, $bmodule, $bcachetime)
{
    global $xoopsUser;
    if (empty($bmodule)) {
        xoops_cp_header();
    ...
    //$myblock->setVar('name', $bname.'-'.$name);
    $newid = $myblock->store();
    if (!$newid) {
        xoops_cp_header();
...
function edit_block($bid)
{
    ...
    // block call back code gen
echo "<h2>"._AM_BLOCK_CALLBACK."</h2>";
echo <<<EOF
<pre style="border:1px dotted #ccc;padding:3px;">
<{freeBlock id={$bid}}>
</pre>
EOF;
    if ( !$block['is_custom'] ) {
        ...
        $tplfile =& $tplfile_handler->get($btemplate[0]->getVar('tpl_id'), true);
        $tplsource =  $tplfile->getVar('tpl_source', 'E');
echo <<<EOF
<pre style="border:1px dotted #ccc;padding:3px;">
<{freeBlkTpl id={$bid}}>
$tplsource
...
function update_block($bid, $bside, $bweight, $bvisible, $btitle, $bname, $bcontent, $bctype, $bcachetime, $bmodule, $options=array())
{
    global $xoopsConfig;
    if (empty($bmodule)) {
        xoops_cp_header();
...
function clone_block_ok($bid, $bside, $bweight, $bvisible, $btitle, $bname, $bcontent, $bcachetime, $bmodule, $options = array())
{
    global $xoopsUser;
    /*
    if (empty($bmodule)) {
